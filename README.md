...
| 服务端 | Java 8 · Spring Boot 2.7.18 · MyBatis-Plus 3.5.3.1 · MySQL 8.0 · Redis · JWT(jjwt) | 业务核心，统一 `Result<T>` 返回；Redis 列表缓存 + 计数；JWT 无状态鉴权 |
...
- **商品详情**：图片轮播、卖家卡片（信用分）、相关商品推荐、浏览量实时累加
- **收藏**：收藏/取消，联合唯一键防重复收藏，Redis 实时维护收藏数
- **即时通讯**：会话列表（最后消息 + 未读红点）、一对一聊天（文字/图片气泡）、消息已读/未读回执
- **订单履约**：订单状态机（待付款 → 待发货 → 待收货 → 已完成 / 已取消）+ 状态流转日志
- **管理后台**：仪表盘统计、商品/订单/用户/分类管理
- **Redis 缓存与计数**：商品列表 cache-aside 缓存（发布/改状态/删除主动失效）、浏览量/收藏量「DB 基础值 + Redis 增量」实时计数，定时任务回写 DB
- **登录鉴权**：微信 `wx.login` → `code2session` 换 openid → 签发 JWT；`JwtInterceptor` 统一拦截 `/api/**`，受保护接口从 token 解析用户身份（无 appid/secret 时 dev-mode 兜底）

## 项目架构

```
┌──────────────┐    HTTP    ┌──────────────────────┐    JDBC    ┌──────────────┐
│  微信小程序    │ ─────────▶ │   Spring Boot 服务端   │ ─────────▶ │    MySQL 8   │
...
│   │   ├── controller/            # 8 个接口控制器
│   │   ├── service/  service/impl/ # 业务层（含 RedisService / WxService）
│   │   ├── mapper/  entity/       # MyBatis-Plus
│   │   ├── config/                # CORS / MyBatis-Plus / JWT 拦截器 / 静态资源映射
│   │   ├── common/                # Result / 全局异常 / UserContext
│   │   └── util/                  # JwtUtil
│   └── src/main/resources/
│       ├── db/schema.sql          # 建表脚本
│       ├── db/seed.sql            # 种子数据
│       └── application.yml.example# 配置示例（脱敏，含 Redis/JWT/微信配置）
...
| 表 | 说明 |
|----|------|
| `user` | 用户（昵称、头像、信用分、学校、openid） |
| `category` | 商品分类 |
| `product` | 商品（价格、新旧、交易方式、状态、浏览量、收藏量） |
...
- JDK 8+、Maven 3.6+
- MySQL 8.0
- Redis（本机 `localhost:6379`，用于列表缓存与浏览/收藏计数）
- Node.js 16+（仅管理后台需要）
- 微信开发者工具
...
确保本机 Redis 已在 6379 端口运行（Windows 可下载 Redis for Windows 或使用 WSL/Docker）。验证：
...
# 编辑 application.yml，将 datasource.password 改为你的 MySQL 密码
mvn spring-boot:run
```
...
| 小程序 | 开发模式测试用户 | `wechat.dev-mode=true` 时用固定测试 openid 自动注册/登录，无需真实 AppID |

## Redis 缓存与计数说明

- **商品列表缓存**：`product:list:*`，cache-aside 模式，TTL 300s；发布商品、修改商品状态、删除商品时主动失效（`evictListCache`）。
- **浏览量**：`product:view:*`，打开商品详情时 `INCR`；展示时 = DB 基础值 + Redis 增量。
- **收藏量**：`product:fav:*`，收藏/取消收藏时 `INCR(+1/-1)`。
- **定时回写**：`@Scheduled` 每 5 分钟把浏览/收藏增量回写 MySQL 并清空对应 key（`RedisService.flushCountersToDb`）。

可随时用 `redis-cli` 验证：

```bash
redis-cli KEYS 'product:*'
redis-cli GET 'product:view:1'          # 商品 id=1 的浏览增量
redis-cli GET 'product:fav:1'           # 商品 id=1 的收藏增量
redis-cli TTL 'product:list:1:10:all:latest:'   # 列表缓存剩余存活时间
```
...
- 登录采用 JWT 无状态鉴权：微信 `wx.login` → `code2session` 换 openid → 自动注册 → 签发 token；`wechat.dev-mode=true` 或无 appid/secret 时回退到固定测试 openid。
